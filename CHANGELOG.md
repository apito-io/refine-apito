...
- **Data provider caching**: drop urql `cacheExchange`; use `fetchExchange` with `requestPolicy: 'network-only'` so list/detail views reflect fresh Apito data after mutations.

### Changed

- **Naming golden vectors**: expand `src/fixtures/naming_vectors.json` for cross-SDK parity with js-admin-sdk.

## [1.0.2] - 2026-05-20

### Fixed

- **`createMany` / CSV import upsert**: `$connect` variable type now uses `{Model}_Relation_Connect_Payload` (same as `create` / `update`) instead of `{Model}_List_Connect`, matching Apito `upsert*List` mutations.

## [1.0.1] - 2026-05-13

### Fixed

- **Data provider filters**: reject dynamic filter keys `__proto__`, `constructor`, and `prototype` so user-controlled GraphQL variables cannot pollute object prototypes when building `meta` / `where` maps.
